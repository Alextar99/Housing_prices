...
# --- PASO 6.5: Justificación empírica de k = 5 (multicriterio) ---
#
# Metodología:
#   1. Submuestra de 500 observaciones completas.
#   2. Se introduce un 10% de NAs artificiales por variable.
#   3. Se imputa con cada k candidato.
#   4. Se repite 20 veces.
#   5. Se evalúan tres ejes:
#        · Precisión (NRMSE): error medio por valor imputado
#        · Robustez (perturbación): se añade ruido gaussiano a la
#          matriz de distancias y se re-imputa. La diferencia media
#          entre ambas imputaciones mide cuán sensible es k a
#          pequeños cambios en los datos. k=1 depende de un único
#          vecino: cualquier perturbación puede cambiarlo. k=5 usa
#          la mediana de 5 vecinos, que absorbe el ruido.
#        · Fidelidad distribucional: media de tres sub-criterios
#          normalizados (preservación de varianza, asimetría y
#          estructura de correlaciones).
#   6. Cada métrica se normaliza a [0,1] (0=mejor, 1=peor).
#      A diferencia de los rankings, esto respeta la MAGNITUD
#      de las diferencias: la penalización de robustez para k=1
#      es enorme, mientras que las diferencias distribucionales
#      entre k=3 y k=5 son mínimas.
#   7. Puntuación global ponderada: Precisión (peso 1), Robustez
#      (peso 2), Fidelidad (peso 1). La robustez recibe el doble
#      porque todo el análisis posterior (PCA, clustering,
#      correlaciones) depende de que la imputación sea estable
#      ante pequeñas variaciones en los datos.
# ----------------------------------------------------------------

cat("\n", strrep("=", 65), "\n")
cat("PASO 6.5 — SELECCIÓN MULTICRITERIO DEL PARÁMETRO k\n")

cat(strrep("=", 65), "\n")

k_candidatos <- c(1, 3, 5, 7, 10, 15)
pct_na_sim   <- 0.10
n_rep        <- 20
n_sub        <- 500
perturb_sd   <- 2.0  # magnitud del ruido en distancias

# Pesos (configurables)
w_precision <- 1   # NRMSE
w_robustez  <- 2   # sensibilidad a perturbaciones
w_fidelidad <- 1   # preservación distribucional

# --- Submuestra de observaciones completas ---
idx_completos  <- complete.cases(train[, out_vars])
train_completo <- train[idx_completos, ]

set.seed(123)
if (nrow(train_completo) > n_sub) {
  idx_sub        <- sample(seq_len(nrow(train_completo)), n_sub)
  train_completo <- train_completo[idx_sub, ]
}

# --- Matriz numérica pura ---
mat_orig <- matrix(NA_real_, nrow = nrow(train_completo), ncol = length(out_vars))
colnames(mat_orig) <- out_vars
for (j in seq_along(out_vars)) {
  mat_orig[, j] <- as.double(train_completo[[ out_vars[j] ]])
}
stopifnot(all(is.finite(mat_orig)))

n_obs <- nrow(mat_orig)
n_v   <- ncol(mat_orig)

# --- Estadísticos de referencia ---
ref_var   <- apply(mat_orig, 2, var)
ref_rango <- apply(mat_orig, 2, function(x) max(x) - min(x))
ref_cor   <- cor(mat_orig)
...
ref_skew <- apply(mat_orig, 2, calc_skew)

# --- Matriz de distancias (una sola vez) ---
mat_sc <- scale(mat_orig)
mat_sc[!is.finite(mat_sc)] <- 0
dist_mat <- as.matrix(dist(mat_sc))

cat(sprintf("Observaciones: %d | NAs artificiales: %.0f%% | Reps: %d\n",
            n_obs, pct_na_sim * 100, n_rep))
cat(sprintf("Perturbación SD: %.1f | Pesos: Precisión=%d, Robustez=%d, Fidelidad=%d\n\n",
            perturb_sd, w_precision, w_robustez, w_fidelidad))

# --- Simulación ---
set.seed(123)
lista_res <- vector("list", n_rep * length(k_candidatos))
idx_res   <- 0L

for (rep in 1:n_rep) {
  
  # NAs artificiales (mismos para todos los k)
  mat_na   <- mat_orig
  idx_list <- vector("list", n_v)
  for (j in 1:n_v) {
    n_na <- ceiling(pct_na_sim * n_obs)
    idx_list[[j]] <- sample.int(n_obs, n_na)
    mat_na[idx_list[[j]], j] <- NA
  }
  
  # Perturbación simétrica de la matriz de distancias (misma para todos los k)
  noise <- matrix(rnorm(n_obs * n_obs, 0, perturb_sd), n_obs, n_obs)
  noise <- (noise + t(noise)) / 2
  diag(noise) <- 0
  dist_pert <- pmax(dist_mat + noise, 0)
  
  for (ki in seq_along(k_candidatos)) {
    k_val <- k_candidatos[ki]
    
    mat_imp  <- mat_na   # imputación con distancias originales
    mat_pimp <- mat_na   # imputación con distancias perturbadas
    
    for (j in 1:n_v) {
      for (i in idx_list[[j]]) {
        otros  <- (1:n_obs)[-i]
        k_real <- min(k_val, length(otros))
        
        vec_o <- otros[order(dist_mat[i, otros])[1:k_real]]
        mat_imp[i, j] <- median(mat_orig[vec_o, j])
        
        vec_p <- otros[order(dist_pert[i, otros])[1:k_real]]
        mat_pimp[i, j] <- median(mat_orig[vec_p, j])
      }
    }
    ...
    nrmse_val <- tryCatch({
      vals <- numeric(n_v)
      for (j in 1:n_v) {
        r <- mat_orig[idx_list[[j]], j]
        p <- mat_imp[idx_list[[j]], j]
        vals[j] <- sqrt(mean((p - r)^2)) / max(ref_rango[j], 1e-10)
      }
      mean(vals)
    }, error = function(e) NA_real_)
    
    # --- Robustez (sensibilidad a perturbación) ---
    # Diferencia media normalizada entre imputación original y perturbada.
    # Un k bajo produce valores muy distintos al perturbar las distancias;
    # un k alto, gracias a la mediana, absorbe el ruido.
    robust_val <- tryCatch({
      vals <- numeric(n_v)
      for (j in 1:n_v) {
        o <- mat_imp[idx_list[[j]], j]
        p <- mat_pimp[idx_list[[j]], j]
        vals[j] <- mean(abs(o - p)) / max(ref_rango[j], 1e-10)
      }
      mean(vals)
    }, error = function(e) NA_real_)
    ...
    vardev_val <- tryCatch({
      vals <- numeric(n_v)
      for (j in 1:n_v) {
        vals[j] <- abs(var(mat_imp[, j]) / max(ref_var[j], 1e-10) - 1)
      }
      mean(vals)
    }, error = function(e) NA_real_)
    
    # --- Preservación de asimetría ---
    skew_val <- tryCatch({
      vals <- numeric(n_v)
      for (j in 1:n_v) {
        vals[j] <- abs(calc_skew(mat_imp[, j]) - ref_skew[j])
      }
      mean(vals)
    }, error = function(e) NA_real_)
    ...
    cor_val <- tryCatch({
      sqrt(mean((cor(mat_imp) - ref_cor)^2))
    }, error = function(e) NA_real_)
    
    idx_res <- idx_res + 1L
    lista_res[[idx_res]] <- data.frame(
      Rep = rep, k = k_val,
      NRMSE = nrmse_val, Robustez = robust_val,
      VarRatio_dev = vardev_val, SkewDiff = skew_val, CorDist = cor_val
    )
  }
}

resultados_k <- do.call(rbind, lista_res)

# --- Filtro de seguridad ---
mask_ok <- apply(resultados_k[, 3:7], 1, function(r) all(is.finite(r)))
if (sum(!mask_ok) > 0) cat(sprintf("Iteraciones descartadas: %d\n", sum(!mask_ok)))
resultados_k <- resultados_k[mask_ok, ]

# --- Resumen ---
resumen_k <- resultados_k %>%
  group_by(k) %>%
  summarise(
    NRMSE        = mean(NRMSE),
    Robustez     = mean(Robustez),
    VarRatio_dev = mean(VarRatio_dev),
    SkewDiff     = mean(SkewDiff),
    CorDist      = mean(CorDist),
    .groups      = "drop"
  )

cat("--- Métricas por k (valores medios) ---\n")
print(resumen_k %>% mutate(across(where(is.numeric), ~round(.x, 5))),
      row.names = FALSE)

# --- Puntuación normalizada [0,1] ---
# A diferencia de los rankings, las puntuaciones normalizadas respetan
# la magnitud real: una diferencia de 0.001 en varianza pesa mucho
# menos que una diferencia de 0.03 en robustez.
normalizar <- function(x) {
  rng <- max(x) - min(x)
  if (rng < 1e-12) return(rep(0, length(x)))
  (x - min(x)) / rng
}

resumen_k <- resumen_k %>%
  mutate(
    NRMSE_n    = normalizar(NRMSE),
    Robust_n   = normalizar(Robustez),
    VarRatio_n = normalizar(VarRatio_dev),
    Skew_n     = normalizar(SkewDiff),
    Cor_n      = normalizar(CorDist),
    # Fidelidad = media de los tres sub-criterios distribucionales
    Fidelidad  = (VarRatio_n + Skew_n + Cor_n) / 3,
    # Puntuación global ponderada (menor = mejor)
    Score      = (w_precision * NRMSE_n +
                    w_robustez  * Robust_n +
                    w_fidelidad * Fidelidad) /
      (w_precision + w_robustez + w_fidelidad)
  ) %>%
  arrange(Score)

cat("\n--- Puntuación normalizada (0 = mejor, 1 = peor) ---\n")
cat(sprintf("Pesos: Precisión=%d | Robustez=%d | Fidelidad=%d\n\n",
            w_precision, w_robustez, w_fidelidad))
print(resumen_k %>%
        dplyr::select(k, NRMSE_n, Robust_n, Fidelidad, Score) %>%
        mutate(across(where(is.numeric), ~round(.x, 3))),
      row.names = FALSE)

k_optimo <- resumen_k$k[1]
cat(sprintf("\n→ k seleccionado: k = %d (puntuación = %.3f)\n", k_optimo, resumen_k$Score[1]))
cat("\n  Interpretación del resultado:\n")
cat("  · k = 1: excelente fidelidad distribucional (no promedia nada),\n")
cat("    pero extremadamente sensible a perturbaciones: cambiar un\n")
cat("    solo dato puede alterar completamente los valores imputados.\n")
cat("  · k = 3: buena precisión, pero la mediana de 3 vecinos sigue\n")
cat("    siendo sensible al cambio de un solo vecino.\n")
cat("  · k = 5: la mediana de 5 vecinos absorbe el ruido (robustez),\n")
cat("    con una pérdida distribucional mínima respecto a k = 1.\n")
cat("  · k ≥ 7: mayor robustez pero sobre-suaviza, distorsionando\n")
cat("    varianza, asimetría y correlaciones.\n")

# --- Heatmap de puntuaciones normalizadas ---
# Usamos un vector con nombre en lugar de recode() (deprecado en dplyr)
etiquetas_criterio <- c(
  NRMSE_n    = "Precisión\n(NRMSE)\npeso: 1",
  Robust_n   = "Robustez\n(perturbación)\npeso: 2",
  VarRatio_n = "Preservación\nVarianza",
  Skew_n     = "Preservación\nAsimetría",
  Cor_n      = "Preservación\nCorrelaciones"
)

heatmap_data <- resumen_k %>%
  dplyr::select(k, NRMSE_n, Robust_n, VarRatio_n, Skew_n, Cor_n) %>%
  pivot_longer(cols = -k, names_to = "Criterio", values_to = "Score_norm") %>%
  mutate(
    Criterio = etiquetas_criterio[Criterio],
    Criterio = factor(Criterio, levels = etiquetas_criterio),
    k_label  = paste0("k = ", k)
  )

p_multi <- ggplot(heatmap_data,
                  aes(x = Criterio, y = reorder(k_label, -k), fill = Score_norm)) +
  geom_tile(color = "white", linewidth = 1.2) +
  geom_text(aes(label = round(Score_norm, 2)), size = 4, fontface = "bold") +
  scale_fill_gradient2(low = "#1A9641", mid = "#FFFFBF", high = "#D7191C",
                       midpoint = 0.5, name = "Score\n(0 = mejor)") +
  labs(
    title    = "Selección multicriterio del parámetro k — Puntuaciones normalizadas",
    subtitle = sprintf(
      "k = %d: mejor equilibrio entre precisión, robustez y fidelidad distribucional",
      k_optimo),
    x = NULL, y = NULL
  ) +
  theme_hp +
...
# Columnas auxiliares (dummies + z-scores)
cols_auxiliares <- grep("(_z$|^Neighborhood_|^BldgType_|^SaleCondition_)", 
                        names(train), value = TRUE)

train_resumen <- train %>% dplyr::select(-all_of(cols_auxiliares))

cat(sprintf("Variables totales         : %d\n",  ncol(train_resumen)))
cat(sprintf("  - Numéricas             : %d\n",  sum(sapply(train_resumen, is.numeric))))
cat(sprintf("  - Factor / Ordinal      : %d\n",  sum(sapply(train_resumen, is.factor))))
cat(sprintf("  - Valores perdidos      : %d\n",  sum(is.na(train_resumen))))
...
# Incluyendo variables categóricas ordinales
# ------------------------------------------------------------------
num_vars_spearman <- c(
  ...
  "WoodDeckSF", "OpenPorchSF", "QualSF"
)

ord_vars_spearman <- intersect(c(
...
train_cor <- train %>%
  dplyr::select(all_of(c(num_vars_spearman, ord_vars_spearman))) %>%
  mutate(
    across(all_of(ord_vars_spearman), ~ as.numeric(.x))
  )
...
corrplot(
  ...
  title = "Matriz de correlación de Spearman — Numéricas + categóricas ordinales",
  mar = c(0, 0, 2, 0)
)
...
# ------------------------------------------------------------------
# FUNCIONES AUXILIARES: SISTEMATIZACIÓN DE TABLAS Y ASOCIACIÓN
# ------------------------------------------------------------------
interpretar_fuerza <- function(x) {
  case_when(
    is.na(x) ~ NA_character_,
    abs(x) < 0.10 ~ "Insignificante",
    abs(x) < 0.30 ~ "Débil",
    abs(x) < 0.50 ~ "Moderada",
    TRUE ~ "Fuerte"
  )
...
extraer_test_asociacion <- function(tabla) {
  chi_aux <- suppressWarnings(chisq.test(tabla, correct = FALSE))
  pct_ok <- 100 * mean(chi_aux$expected >= 5)
  
  if (pct_ok >= 80) {
    tst <- suppressWarnings(chisq.test(tabla, correct = FALSE))
    tibble(
      Metodo = "Chi-cuadrado de Pearson",
      Estadistico = unname(tst$statistic),
      gl = unname(tst$parameter),
      p_valor = tst$p.value,
      Pct_esperadas_ge_5 = round(pct_ok, 1)
    )
  } else {
    simular <- nrow(tabla) > 2 | ncol(tabla) > 2
    tst <- fisher.test(tabla, simulate.p.value = simular, B = 10000)
    tibble(
      Metodo = ifelse(simular, "Fisher exacto (Monte Carlo)", "Fisher exacto"),
...
resumen_tabla_contingencia <- function(tabla, digits = 3) {
  list(
    absoluta = addmargins(tabla),
    relativa_conjunta = round(prop.table(tabla), digits),
    relativa_fila = round(prop.table(tabla, margin = 1), digits),
    relativa_columna = round(prop.table(tabla, margin = 2), digits)
  )
}

medidas_nominales <- function(tabla) {
  salida <- tibble(
    Medida = c("C de Pearson", "V de Cramer"),
    Valor = c(
      as.numeric(DescTools::ContCoef(tabla)),
      as.numeric(DescTools::CramerV(tabla))
    )
  )
  ...
  if (all(dim(tabla) == c(2, 2))) {
    ...
    q_yule <- if ((a * d + b * c_) == 0) NA_real_ else (a * d - b * c_) / (a * d + b * c_)
    
    salida <- bind_rows(
      salida,
      tibble(
        Medida = c("Phi", "Q de Yule"),
        Valor = c(
          as.numeric(DescTools::Phi(tabla)),
          q_yule
        )
      )
    )
  ...
  salida %>%
    mutate(Fuerza = interpretar_fuerza(Valor))
}

medidas_ordinales <- function(tabla) {
  gamma <- DescTools::GoodmanKruskalGamma(tabla, conf.level = 0.95)
  tau_b <- DescTools::KendallTauB(tabla, conf.level = 0.95)
  tau_c <- DescTools::StuartTauC(tabla, conf.level = 0.95)
  som_r <- DescTools::SomersDelta(tabla, direction = "row", conf.level = 0.95)
  som_c <- DescTools::SomersDelta(tabla, direction = "column", conf.level = 0.95)
  ...
  tibble(
    ...
    Valor = c(gamma[1], tau_b[1], tau_c[1], som_r[1], som_c[1]),
    LI95 = c(gamma[2], tau_b[2], tau_c[2], som_r[2], som_c[2]),
    LS95 = c(gamma[3], tau_b[3], tau_c[3], som_r[3], som_c[3])
  ) %>%
    mutate(Fuerza = interpretar_fuerza(Valor))
}

analizar_tabla_asociacion <- function(tabla, nombre, tipo = c("nominal", "ordinal", "mixta"), digits = 3) {
  tipo <- match.arg(tipo)
  ...
  resumen_tab <- resumen_tabla_contingencia(tabla, digits = digits)
  cat("\nTabla con totales:\n"); print(resumen_tab$absoluta)
  cat("\nFrecuencias relativas conjuntas:\n"); print(resumen_tab$relativa_conjunta)
  cat("\nFrecuencias relativas por fila:\n"); print(resumen_tab$relativa_fila)
  cat("\nFrecuencias relativas por columna:\n"); print(resumen_tab$relativa_columna)
  
  test_tbl <- extraer_test_asociacion(tabla)
  cat("\nResumen del test de independencia:\n"); print(test_tbl)
  
  medidas_tbl <- switch(
    tipo,
    nominal = medidas_nominales(tabla),
    ordinal = medidas_ordinales(tabla),
    mixta = medidas_nominales(tabla)
  )
...
res_2x2 <- analizar_tabla_asociacion(
  tabla = con_2x2,
  nombre = "c.2-c.3) CentralAir × PriceCat",
  tipo = "nominal"
)

chi_2x2 <- suppressWarnings(chisq.test(con_2x2, correct = FALSE))
v_2x2_dt <- DescTools::CramerV(con_2x2)
...
res_NxM <- analizar_tabla_asociacion(
  tabla = con_NxM,
  nombre = "c.4-c.5) Neighborhood × PriceCat",
  tipo = "nominal"
)

chi_NxM <- suppressWarnings(chisq.test(con_NxM, correct = FALSE))
v_NxM <- DescTools::CramerV(con_NxM)
...
res_ord <- analizar_tabla_asociacion(
  tabla = con_ord,
  nombre = "c.6) QualCat × AgeCat",
  tipo = "ordinal"
)
...
res_mixta <- analizar_tabla_asociacion(
  tabla = con_mixta,
  nombre = "c.8) BldgType × QualCat",
  tipo = "mixta"
)

chi_mixta <- suppressWarnings(chisq.test(con_mixta, correct = FALSE))
...
# ==============================================================
# c.9) SISTEMATIZACIÓN MASIVA DE MEDIDAS DE ASOCIACIÓN
# ==============================================================
cat("\n", strrep("-", 65), "\n")
cat("--- c.9) Análisis sistemático de todas las variables categóricas ---\n")
cat(strrep("-", 65), "\n")

# Función auxiliar robusta: elimina niveles vacíos para evitar divisiones por cero (NaN)
calcular_v_cramer <- function(tabla) {
  # Filtro clave: Eliminar filas y columnas que no tengan ninguna observación
  ...
  return(as.numeric(sqrt(chi_sq / (n * min_dim))))
}

vars_categoricas <- train %>%
  dplyr::select(where(~ is.factor(.) || is.character(.))) %>%
  names() %>%
  setdiff(c("PriceCat", "SalePrice", "Id"))

asociaciones_masivas <- map_dfr(vars_categoricas, function(var) {
  tabla <- table(train[[var]], train$PriceCat)
  
  if (nrow(tabla) > 1 && ncol(tabla) > 1) {
    v_val <- calcular_v_cramer(tabla)
    tibble(Variable = var, V_Cramer = v_val)
  } else {
    tibble(Variable = var, V_Cramer = NA_real_)
  }
}) %>%
  drop_na() %>%
  arrange(desc(V_Cramer)) %>%
  mutate(
    Tipo_Asociacion = case_when(
...
cat("\nTop 15 variables categóricas asociadas con PriceCat:\n")
print(head(asociaciones_masivas, 15))
...
# ==============================================================
cat("\n", strrep("-", 65), "\n")
cat("--- c.10) Comparación visual de los 5 pares clave ---\n")
cat(strrep("-", 65), "\n")

pares_especificos <- tibble(
  Par = c(
    "CentralAir × PriceCat",
    "Neighborhood × PriceCat",
    "QualCat × AgeCat",
    "OverallQual × KitchenQual",
    "BldgType × QualCat"
  ),
  Var1 = c("CentralAir", "Neighborhood", "QualCat", "OverallQual", "BldgType"),
  Var2 = c("PriceCat", "PriceCat", "AgeCat", "KitchenQual", "QualCat")
)

pares_especificos <- pares_especificos %>%
  mutate(
    Valor = map2_dbl(Var1, Var2, function(v1, v2) {
      if(v1 %in% names(train) & v2 %in% names(train)) {
        tabla <- table(train[[v1]], train[[v2]])
        calcular_v_cramer(tabla)
      } else {
        NA_real_
      }
    }),
...
p_5_pares <- ggplot(pares_especificos, aes(x = reorder(Par, Valor), y = Valor, fill = Tipo_Asociacion)) +
  geom_col(width = 0.6, color = "black", alpha = 0.85) +
  geom_text(aes(label = sprintf("%.3f", Valor)), hjust = -0.2, size = 4, fontface = "bold") +
  geom_hline(yintercept = 0.50, linetype = "solid", color = "gray20") +
  geom_hline(yintercept = 0.30, linetype = "dashed", color = "gray40") +
  ...
  scale_fill_manual(values = c(
    ...
    "Moderada" = "#FDAE61", 
    "Débil" = "#74ADD1",
    "Insignificante" = "#E0E0E0"
  )) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)), limits = c(0, 1)) +
  labs(
    title = "Fuerza de asociación en los 5 pares clave (V de Cramer)",
    subtitle = "Umbrales: <0.1 Insignificante | <0.3 Débil | <0.5 Moderada | >0.5 Fuerte",
    x = NULL, y = "Magnitud de la V de Cramer"
  ) +
  theme_hp +
...
# ==============================================================
# c.11) TOP 15 ASOCIACIONES: ANTIGÜEDAD, BARRIO Y CALIDAD
# ==============================================================
cat("\n", strrep("-", 65), "\n")
cat("--- c.11) Sistematización para Antigüedad, Barrio y Calidad ---\n")
cat(strrep("-", 65), "\n")

buscar_top15_asociaciones <- function(target_name, titulo_amigable) {
  if (!target_name %in% names(train)) {
    cat(sprintf("\n[!] La variable '%s' no se encuentra.\n", target_name))
    return(NULL)
  }
  ...
  vars_evaluar <- train %>%
    dplyr::select(where(~ is.factor(.) || is.character(.))) %>%
    names() %>%
    setdiff(c(target_name, "SalePrice", "Id", "PriceCat", "QualCat", "AgeCat")) 
  
  resultados <- map_dfr(vars_evaluar, function(var) {
    tabla <- table(train[[var]], train[[target_name]])
    v_val <- calcular_v_cramer(tabla)
    tibble(Variable = var, V_Cramer = v_val)
  }) %>%
    drop_na() %>%
  ...
  cat(sprintf("\n======================================================\n"))
  cat(sprintf(" TOP 15 VARIABLES ASOCIADAS CON: %s (%s)\n", titulo_amigable, target_name))
  cat(sprintf("======================================================\n"))
  print(resultados)
...
top15_edad <- buscar_top15_asociaciones("AgeCat", "Antigüedad de la vivienda")
top15_barrio <- buscar_top15_asociaciones("Neighborhood", "Barrio")
top15_calidad <- buscar_top15_asociaciones("OverallQual", "Calidad general")
